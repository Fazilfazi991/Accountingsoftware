-- Carry forward the Supplier Payment ambiguity repair and make only the
-- open-item status CASE explicitly resolve to the existing enum.
...
begin
  ...
  for x in select value from jsonb_array_elements(p_allocations) loop
    ...
    select target_open_item.* into v_target_open_item
     ...
     for update;
    if not found or v_target_open_item.kind <> 'payable' or v_target_open_item.supplier_id <> d.supplier_id or v_amount <= 0 or v_amount > v_target_open_item.remaining_amount then
  ...
  for x in select value from jsonb_array_elements(p_allocations) loop
    ...
    update public.open_items
       set remaining_amount = remaining_amount - v_amount,
           status = case when remaining_amount - v_amount = 0 then 'settled'::public.open_item_status else 'partial'::public.open_item_status end
     where id = (x->>'open_item_id')::uuid;
