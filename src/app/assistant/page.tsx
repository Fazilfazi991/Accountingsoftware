...
export default async function AssistantPage() {
  ...
  return <ProductWorkspace context={context.payload} route="/assistant">
    <AssistantChat key={`${context.organization.id}:${context.branch.id}`} organization={context.organization.name} branch={context.branch.name} providerConfigured={providerConfigured} />
  </ProductWorkspace>;
}
