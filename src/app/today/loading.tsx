...
export default function Loading() {
  return <main className={styles.page} aria-label="Loading Today">
    ...
    <div className={styles.moneyGrid}>{[1, 2, 3, 4].map((id) => <div className={styles.skeletonCard} key={id} />)}</div>
    <div className={styles.contentGrid}>{[1, 2, 3, 4, 5].map((id) => <div className={styles.skeletonCard} key={id} />)}</div>
  </main>;
}
