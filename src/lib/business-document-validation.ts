...
export function documentValidationMessage(
  ...
  if (kind !== "bill") return "Enter a party, valid dates, and at least one valid line.";

  const messages = issues.map((issue) => {
    const [field, index, lineField] = issue.path;
    if (field === "partyId") return "Supplier is required or invalid.";
    if (field === "documentDate") return "Bill date is required or invalid.";
    ...
    if (field === "lines" && typeof index === "number") {
      ...
      switch (lineField) {
        ...
        case "discount": return prefix + "Discount must be zero or greater.";
    ...
    return "Review the bill details and line values.";
  });
  return [...new Set(messages)].join(" ") || "Review the bill details and line values.";
}
