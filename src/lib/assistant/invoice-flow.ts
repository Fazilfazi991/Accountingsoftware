...
export type InvoiceStage = "idle" | "customer" | "items" | "details" | "preview" | "saving" | "success" | "uncertain";
...
export type InvoiceEvent =
  | { type: "begin"; today: string; requestId: string; firstLine?: BusinessDocumentLine }
...
export const emptyInvoiceState: GuidedInvoiceState = {
  ...
  requestId: "", savedId: "", savedNumber: "", error: "",
};

export function guidedInvoiceReducer(state: GuidedInvoiceState, event: InvoiceEvent): GuidedInvoiceState {
  switch (event.type) {
    case "begin": return { ...emptyInvoiceState, stage: "customer", documentDate: event.today,
    ...
    case "uncertain": return state.stage === "saving" ? { ...state, stage: "uncertain", error: event.error } : state;
