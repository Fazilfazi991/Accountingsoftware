...
export function AiWorkspaceShell({ context, route, children }: {
  ...
  const mode = route === "/assistant" ? "Ask Ledgerly" : "Ask General";
  ...
  return <OrganizationProvider context={context}>
    <div className="app ai-app">
      ...
      <aside className={open ? "ai-sidebar open" : "ai-sidebar"} aria-label="AI workspace navigation">
        <div className="ai-brand"><span className="brand-mark">L</span><b>Ledgerly</b></div>
        ...
        <div className="ai-sidebar-footer"><Link className={route === "/assistant" ? "active" : ""} href="/assistant" onClick={() => setOpen(false)}>Ask Ledgerly</Link><Link className={route === "/general" ? "active" : ""} href="/general" onClick={() => setOpen(false)}>Ask General</Link><Link className="ai-back" href="/today" onClick={() => setOpen(false)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m15 5-7 7 7 7" /><path d="M8 12h11" /></svg> Back to Ledgerly</Link></div>
      </aside>
